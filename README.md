...
Each instance names a set representation (`benchmark`), an operation with its dimension
and device (`instance`, e.g. `matMul-500d-gpu`), how often to repeat it (`repetition`),
where to run it (`device`, `cpu` or `gpu`), and the operation's arguments as JSON
(`params`). Those columns are passed to your scripts in that order, after the interface
version. What each operation must do is defined in the [benchmark
catalog](https://github.com/CORA-COMP/benchmarks).
